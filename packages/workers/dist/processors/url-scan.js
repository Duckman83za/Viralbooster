...
import { scrapeUrl, generatePostsFromUrl } from '@contentos/ai';
export async function urlScanProcessor(job) {
    const { workspaceId, url, platform = 'linkedin', postCount = 5 } = job.data;
    ...
    try {
        ...
        const posts = await generatePostsFromUrl(scrapedContent, apiKey, platform, postCount);
        // 5. Save as drafts (using 'concept' field for source info)
        const createdPosts = await Promise.all(posts.map((content) => prisma.post.create({
            data: {
                workspaceId,
                platform,
                content,
