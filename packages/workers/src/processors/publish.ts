...
import { MockConnector, SocialConnector } from '@contentos/connectors';

export async function publishProcessor(job: Job) {
    ...
    const connector: SocialConnector = new MockConnector();

    const result = await connector.publishText(
        { id: post.id, content: post.content },
        { data: "mock-encrypted", iv: "mock-iv" }
    );
